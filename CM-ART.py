...
def cm_art(M, label, rho, delta=0.01, beta=0.6, save_path_root=''):
    """
    ...
    for n in range(1, row):
        ...
        # ---------------------------------------警戒参数自适应--------------------------------------------------------
        # 此处是CM-ART的核心，思想是：针对当前要进行聚类的样本I
        # 1. 对于所有类簇，选出 匹配函数M大于警戒参数rho的类簇，称为winner候选者集合(如果没有winner候选者,即不存在winner，则CM-ART结束)
        # 2. 计算这些候选者类簇中，对应T最大的类簇（即winner类簇），并在候选者集合中，去除winner类簇，得到剩下的类簇（可称为失败者簇）
        # 3. 把这些失败者类簇的rho改为 rho_new = M + delta
        # （M是大于rho的，因此rho_new是增加，并且能保证 rho_new > M。目的是，此时的输入样本I不会再落在这些失败者簇的范围内，减少了警戒区域的重叠部分）

        # 1.选出 匹配函数M大于警戒参数rho的类簇，称为winner候选者集合
        a = np.where(M_values[0, :] >= rho_0[0, :])  # 返回一个tuple，a[0]是个数组，里面每个元素是索引
        b = a[0]

        # 如果winner候选者集合存在，则执行CM-ART，否则停止CM-ART （winner候选者集合不存在，相当于winner不存在，则CM-ART结束）
        if b.size > 0:
            # 2. 候选者集合中，去除winner类簇，得到剩下的类簇（可称为失败者簇）
...
if __name__ == '__main__':
    ...
    cm_art(wine_data, wine_label, 0.2)
